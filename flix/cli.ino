...
void doCommand(String str, bool echo = false) {
	...
	command.toLowerCase();
	// Allow "crcr" (double type or echo) to run RC calibration
	if (command == "crcr") command = "cr";
	...
	if (command == "help" || command == "motd") {
		...
		print("time: %.1f  mode: %s  armed: %d\n", controlTime, getModeName(), armed);
		...
		if (arg0 != "") {
			glog = arg0.toInt();
			if ((glog < 0 ) || (glog > 4)) glog = 0;
			// Reset lcntr khi thay đổi glog mode
			extern int lcntr;
			lcntr = 0; // Reset để gửi ngay lần đầu
			print("log mode set to %d\n", glog);
		}
		else dumpLog();
	} else if (command == "cr") {
		calibrateRC();
