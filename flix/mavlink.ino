...
extern float z_hold;            // from control.ino
extern float controlTime;
extern float roll_H, pitch_H;   // attitude actually used by controller
extern Vector gyro;             // gyro rates actually used by controller
extern float t;                 // current time
extern bool armed;
extern int mode;            // from control.ino (attitude/rate PIDs)
void resetAltitudeControl();     // from control.ino (only altitude PIDs)

float lastMavlinkTime = 0;      // last MAVLink receive time
int mavlinkSysId = 1;
...
void sendMavlink() {
	...
	if (telemetryFast && mavlinkConnected) {
		...
		// Build quaternion from roll_H / pitch_H (yaw = 0) so QGC shows the same angles as controller uses.
		// Also send raw gyro rates instead of estimated rates.
		Quaternion q_vis = Quaternion::fromEuler(Vector(roll_H, pitch_H, 0.0f));
		mavlink_msg_attitude_quaternion_pack(mavlinkSysId, MAV_COMP_ID_AUTOPILOT1, &msg,
			time,
			q_vis.w, q_vis.x, q_vis.y, q_vis.z,
...
void receiveMavlink() {
    ...
    lastMavlinkTime = t;  // update thời điểm nhận gói
}
...
void handleMavlink(const void *_msg) {
	...
	if (msg.msgid == MAVLINK_MSG_ID_COMMAND_LONG) {
		...
		if (m.command == MAV_CMD_COMPONENT_ARM_DISARM) {
			if (m.param1 && controlThrottle > 0.05f) return; // don't arm if throttle is not low
			accepted = true;
			if (m.param1 == 1) {
				// ARM: behave like original flix – do NOT hard-reset PIDs, just start from current state
				armed = true;
				// Reset throttle on arm so thrust starts at idle, not from stale MANUAL_CONTROL/RC
				controlThrottle = 0.0f;
				thrustTarget = 0.0f;
			} else {
				// DISARM: only reset altitude-related control state
				armed = false;
